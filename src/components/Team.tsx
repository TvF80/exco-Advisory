      ...
      >
        ...
        <div className="bg-[#0A2E1E] p-6 flex items-center gap-5">
          <div className="w-16 h-16 rounded-full overflow-hidden shrink-0 ring-2 ring-[#C9A84C]/50">
            {member.photo ? (
              <img src={member.photo} alt={member.name} className="w-full h-full object-cover" onError={e => {
                (e.target as HTMLImageElement).style.display = 'none';
                (e.target as HTMLImageElement).nextElementSibling?.removeAttribute('style');
              }} />
            ) : null}
            <div className={member.photo ? 'hidden' : 'w-full h-full'} style={{ display: member.photo ? 'none' : undefined }}>
          ...
          <div className="flex gap-3">
            {member.email && (
              <a
                ...
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-[#0A2E1E] text-white rounded-sm text-sm font-medium hover:bg-[#1A5233] transition-colors"
              >
                <Mail size={14} />
                Email
              </a>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl">
          {team.map((member, i) => (
            ...
            >
              {/* Photo */}
              <div className="w-full aspect-[4/5] rounded-lg overflow-hidden mb-5 relative">
              ...
              {/* Info */}
              <h3 className="font-serif text-lg font-bold text-[#0A2E1E] group-hover:text-[#1A5233] transition-colors">
                {member.name}
              </h3>
